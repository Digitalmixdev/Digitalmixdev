...
export const DIGITALMIX_TOOLS_KNOWLEDGE: ToolKnowledgeItem[] = [
  ...
  },
  {
    ...
    categoryNameEn: 'Database Tools',
    categoryNameAr: 'أدوات قواعد البيانات',
    descriptionEn: 'Validate, format, parse, beautify, and minify JSON data with instant syntax error highlighting.',
    descriptionAr: 'فحص وتنسيق وتحليل وتجميل وتصغير بيانات JSON مع اكتشاف أخطاء الصياغة فورياً.',
    capabilitiesEn: [
      'Format & beautify raw or unformatted JSON strings',
      'Validate JSON syntax and point out exact line errors',
      'Minify / compact JSON payloads for efficient network transmission',
      'Visual tree inspector and depth level analysis',
      'Calculate key counts, total lines, characters, and JSON payload size',
      'Convert JSON to CSV or download as formatted .json file',
    ],
    capabilitiesAr: [
      'تنسيق وتجميل نصوص JSON الخام أو غير المرتبة',
      'التحقق من صحة صياغة JSON وتحديد مكان الخطأ ورقم السطر بدقة',
      ...
      'تصدير كملف .json منسق أو تحويله',
    ],
    howToUseEn: [
      ...
      'View error notifications with exact character positions if JSON is invalid.',
    ...
    howToUseAr: [
      ...
      'شاهد تنبيهات الأخطاء في حال وجود خطأ بصياغة JSON مع موقعه بدقة.',
      'انسخ الناتج المنسق أو قم بتنزيله مباشرة.',
    ],
    keywords: ['json', 'format', 'beautify', 'validate', 'minify', 'parse', 'pretty print', 'tree', 'payload', 'api', 'schema'],
    relatedTools: ['csv-json', 'jwt', 'sql-formatter'],
    supportsCodeAnalysis: true,
    codeType: 'json',
  },
  {
  ...
  },
  {
    ...
    relatedTools: ['jwt', 'hash-generator'],
    supportsCodeAnalysis: true,
    codeType: 'base64',
  },
  {
