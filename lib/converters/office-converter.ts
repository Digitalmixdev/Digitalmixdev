...
// ----------------------------------------------------
// 12. Visual Mode PDF to DOCX & Content Detection
// ----------------------------------------------------
export async function generateVisualDocxFromPdf(
  file: File | Blob,
  ...
  let pageImages: { pageNumber: number; blob: Blob; dataUrl: string; width: number; height: number }[] = []
  try {
    pageImages = await renderPdfToImages(file, 'jpg', quality, scale)
  } catch (err) {
    console.warn('renderPdfToImages failed in generateVisualDocxFromPdf:', err)
  }
...
`
  ...
  for (let pageIdx = 0; pageIdx < pageImages.length; pageIdx++) {
    ...
    try {
      const u8 = await toUint8Array(pImg.blob)
      if (u8.length > 0) {
        const imgFilename = `media/image_${pImg.pageNumber}_${relIndex}.jpg`
        const relId = `rId${relIndex++}`
        wordRelsContent += `  <Relationship Id="${relId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="${imgFilename}"/>\n`
        zip.file(`word/${imgFilename}`, u8)

        const cx = 5486400
        const aspect = pImg.height && pImg.width ? pImg.height / pImg.width : 1.414
      ...
      console.warn(`Error processing image for page ${pImg.pageNumber}:`, pageErr)
    }
  }
...
export async function detectPdfHasVisualContent(file: File | Blob): Promise<boolean> {
  try {
    const pdfjsLib = await import('pdfjs-dist')
    const pdf = await getPdfDocumentSafe(pdfjsLib, file)
    ...
    for (let i = 1; i <= checkPages; i++) {
      ...
      // 1. Check embedded image operations
      try {
        const opList = await page.getOperatorList()
        const OPS = pdfjsLib.OPS || (pdfjsLib as any).OPS || {}
        ...
        for (let j = 0; j < opList.fnArray.length; j++) {
          ...
          if (fn === OPS.save) {
            ...
            const width = Math.round(Math.abs(ctm[0]))
            const height = Math.round(Math.abs(ctm[3]))
        ...
        console.warn(`Error checking operator list on page ${i}:`, err)
      }

      // 2. Check table column alignments
      try {
        const textContent = await page.getTextContent({ normalizeWhitespace: true })
        interface RawItem {
          x: number
        ...
        for (const item of textContent.items as any[]) {
          ...
          if (!str) continue
          const tx = item.transform || [12, 0, 0, 12, 0, 0]
          items.push({ x: Math.round(tx[4]), y: Math.round(tx[5]), str })
        }
        ...
        for (const lineItems of lineMap.values()) {
          ...
          if (lineItems.length >= 3 && hasColumnAlignments(lineItems)) {
            return true
          }
        }
      } catch (err) {
        console.warn(`Error checking text content for table alignment on page ${i}:`, err)
      }
    }
  } catch (err) {
    console.warn('Error in detectPdfHasVisualContent:', err)
  }
